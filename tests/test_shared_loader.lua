...
local names = {'mods/cowboybingus/better_stratagem_bounce', 'mods/cowboybingus/hellpod_steering_unlocked',
               ...
               'mods/cowboybingus/vanilla_plus_megapack', 'mods/cowboybingus/corpse_collision_repair'}
...
for _, audio in ipairs({false, true}) do
  for mask = 0, 1023 do
    for _, failure in ipairs({'none', 'lookup', 'first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eighth', 'ninth', 'tenth'}) do
        ...
        env.require = function(name)
            ...
            if present[name] ~= nil then
                ...
                if failure == 'first' and name == names[1] or failure == 'second' and name == names[2]
                    ...
                    or failure == 'ninth' and name == names[9] or failure == 'tenth' and name == names[10] then
...
print('PASS: shared coordinator covers all 1024 mod combinations, lookup/module failure isolation, duplicate loads, update returns and original audio callbacks')
