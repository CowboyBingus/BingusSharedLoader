...
for _, audio in ipairs({false, true}) do
  for mask = 0, 255 do
    for _, failure in ipairs({'none', 'lookup', 'first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eighth'}) do
        ...
        env.require = function(name)
            ...
            if present[name] ~= nil then
                ...
                if failure == 'first' and name == names[1] or failure == 'second' and name == names[2]
                    ...
                    or failure == 'seventh' and name == names[7] or failure == 'eighth' and name == names[8] then
        ...
        env.init()
        assert(env.CowboyBingusModLoader.version == 7 and env.CowboyBingusModLoader.api == 1)
        local previous = env.update
        env.update = function(...)
...
print('PASS: shared coordinator covers all 256 mod combinations, lookup/module failure isolation, duplicate loads, update returns and original audio callbacks')
