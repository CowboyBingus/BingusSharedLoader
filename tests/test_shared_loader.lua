...
for _, audio in ipairs({false, true}) do
  for mask = 0, 511 do
    for _, failure in ipairs({'none', 'lookup', 'first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eighth', 'ninth'}) do
        ...
        env.require = function(name)
            ...
            if present[name] ~= nil then
                ...
                if failure == 'first' and name == names[1] or failure == 'second' and name == names[2]
                    ...
                    or failure == 'ninth' and name == names[9] then
        ...
        env.init()
        assert(env.CowboyBingusModLoader.version == 8 and env.CowboyBingusModLoader.api == 1)
        local previous = env.update
        env.update = function(...)
...
print('PASS: shared coordinator covers all 512 mod combinations, lookup/module failure isolation, duplicate loads, update returns and original audio callbacks')
